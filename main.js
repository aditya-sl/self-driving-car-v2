...
function generateCars(N) {
	...
	const startPoint =
		startPoints.length > 0 ? startPoints[0].center : new Start(100, 100);
	const dir =
		startPoints.length > 0 ? startPoints[0].directionVector : new Start(0, -1);
...
function animate(time) {
	...
	bestCar = cars.find(
		(car) => car.fitness === Math.max(...cars.map((c) => c.fitness))
	);
